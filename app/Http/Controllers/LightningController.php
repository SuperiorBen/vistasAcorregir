...
class LightningController extends Controller
{
    function search(Request $request)
    {
        ...
        $query = DB::table('money_withdrawals', 'mw')
            ->select(DB::raw("mw.id as idtx, wallet, cc.dni, cc.name as nombre, number as telefono, TO_CHAR(mw.created, 'HH24:MI:SS DD/MM/YYYY') as fechamov, mc.symbol, mw.amount as cantidad, mw.commission as comision, mw.status as estado"))
            ...
            $query->where('mw.wallet',  'like',  'LN%');
            $query->where('mw.status',  '=',  'verifying');
        ...
        $data['csv'] = "csv/wallet.searchlLN.{$date_from}.{$date_to}_at_" . date('Y-m-d H.i.s') . '.csv';
        file_put_contents($data['csv'], $csv);

        $data['salidas']=$results;
        return view('lightning',  $data);
    }
}
